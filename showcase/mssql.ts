import { MssqlAdapter, createConnection, sql } from "squn";
...
export async function showcaseMssql(url: string): Promise<Suite> {
  ...
  return runSuite("MSSQL", [
    {
      name: "create table",
      fn: async () => {
        await db.execute(sql`IF OBJECT_ID('squn_showcase_users','U') IS NOT NULL DROP TABLE squn_showcase_users`);
